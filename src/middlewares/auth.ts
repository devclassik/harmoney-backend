...
export interface TokenDto { id: number, email: string, rememberMe: Boolean}
 ...
 * @returns 
 */
export const regenerateToken = async (req: Request, res: Response): Promise<Response> => {
  try {
 ...
 * @returns 
 */
export const authGuard = async ( req: Request<any, any, any, any>, res: Response, next: NextFunction ): Promise<void | Response> => {
    ...
    if (token && tokenType === 'Bearer') {
      ...
      jwt.verify(token, secret, (err, decodedToken) => {
        if (err) {
          ...
          res.locals.user = decodedData.data
          next()
        }
      })
    }
